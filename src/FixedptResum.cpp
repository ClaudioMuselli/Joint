...
std::complex<long double> FixedptResum::Sudakov_th_gggH(std::complex<long double> N, long double xp){
  ...
  const std::complex<long double> lN=2.*_as*beta_0*std::log(Nbar);
  if (_ordres==0) return std::exp(Sud);
  if (_ordres>=1){
    Sud+=1./_as*(Ath1g/beta_0/beta_0*(lN+0.5*(1.-lN)*std::log(1.-lN)+(1.-lN/2.)*std::log(1.-lN/2.)));//Controllare
  }
  if (_ordres>=2){
    ...
    +std::log(1.-lN/2.)+0.5*std::pow(std::log(1.-lN/2.),2))+Ath1g*_LR/beta_0*(lN+0.5*std::log(1.-lN)+std::log(1.-lN/2.))
    -lN*_LF/beta_0*Ath1g-Ath2g/std::pow(beta_0,2)*(lN+0.5*std::log(1.-lN)+std::log(1.-lN/2.))
...
std::complex<long double> FixedptResum::Sudakov_th_gqqH(std::complex<long double> N, long double xp){
 std::complex<long double> Sud(0.,0.);
  ...
  if (_ordres==0) return std::exp(Sud);
  if (_ordres>=1){
    Sud+=1./_as*(1./2./beta_0/beta_0*((Ath1g+Ath1q)*lN+Ath1g*(1.-lN)*std::log(1.-lN)+2.*Ath1q*(1.-lN/2.)*std::log(1.-lN/2.)));
  }
  if (_ordres>=2){
    const long double radxp=std::log(std::pow(std::sqrt(1.+xp)+std::sqrt(xp),2)/xp);
    ...
    -std::pow(std::log(1.-lN/2.),2))+(Ath1g+Ath1q)/2.*_LR/beta_0*(lN+std::log(1.-lN))
    +Ath1q/(2.*beta_0)*_LR*(2.*std::log(1.-lN/2.)-std::log(1.-lN))
    -lN*_LF/beta_0*(Ath1g+Ath1q)/2.-(Ath2g+Ath2q)/(2.*std::pow(beta_0,2))*(lN+std::log(1.-lN))
...
std::complex<long double> FixedptResum::Sudakov_th_qqgH(std::complex<long double> N, long double xp){
  ...
  if (_ordres==0) return std::exp(Sud);
  if (_ordres>=1){
    Sud+=1./_as*(1./beta_0/beta_0*(Ath1q*lN-0.5*(Ath1g-2.*Ath1q)*(1.-lN)*std::log(1.-lN)+Ath1g*(1.-lN/2.)*std::log(1.-lN/2.)));
  }
  if (_ordres>=2){
    const long double radxp=std::log(std::pow(std::sqrt(1.+xp)+std::sqrt(xp),2)/xp);
    ...
    -std::pow(std::log(1.-lN/2.),2))+Ath1q*_LR/beta_0*(lN+std::log(1.-lN))
    +Ath1g/(2.*beta_0)*_LR*(2.*std::log(1.-lN/2.)-std::log(1.-lN))
    -lN*_LF/beta_0*Ath1q-Ath2q/std::pow(beta_0,2)*(lN+std::log(1.-lN))
...
std::complex<long double> FixedptResum::LOqqgH(std::complex<long double> N, long double xp){
  ...
  std::complex<long double> half(0.5,0.);
  std::complex<long double> xprad(std::pow(std::sqrt(1.+xp)-std::sqrt(xp),4.),0.);
  CLOqqgH=2.*_as*_Cf*_Cf/std::sqrt(M_PIl)*1./(std::pow(std::sqrt(1.+xp)+std::sqrt(xp),2.))*std::exp(LogGamma(N)-LogGamma(N+0.5))
...
long double FixedptResum::Hth1gggH(long double xp){
  ...
  *std::log(xp/(1.+xp))+0.5*_Ca*std::pow(std::log(1.+std::sqrt(xp/(1.+xp))),2)
  ...
  -((_Ca-_Nf)*(std::sqrt(xp)*std::sqrt(1.+xp)*(1.+xp)-2.*xp-xp*xp))/(6.*(1.+8.*xp+9.*xp*xp))+2.*beta_0*M_PIl*_LF-3.*beta_0*M_PIl*_LR;
  return g1gg/M_PIl;
}
long double FixedptResum::Hth1gqqH(long double xp){
  ...
  +beta_0*M_PIl*_LF+3./4.*_Cf*_LF-3.*beta_0*M_PIl*_LR;
  return g1gq/M_PIl;
}
long double FixedptResum::Hth1qqgH(long double xp){
  ...
  +2.*_Ca*gsl_sf_dilog(2.*std::sqrt(xp)/(std::sqrt(1.+xp)+std::sqrt(xp)))+3./2.*_Cf*_LF-3.*beta_0*M_PIl*_LR;
  return g1qq/M_PIl;
}
...
std::vector<std::complex<long double>> FixedptResum::ComputeFixedptResum(std::complex<long double> N, long double xp){
  ...
  switch(_ordres){
    ...
    case(2):{
      if ((_channel==0)||(_channel==1)) 
	ris.push_back(LOgggH(N,xp)*(1.+_as*Hth1gggH(xp))*Sudakov_th_gggH(N,xp));
      else ris.push_back((0.,0.));
      if ((_channel==0)||(_channel==2)) 
	ris.push_back(LOgqqH(N,xp)*(1.+_as*Hth1gqqH(xp))*Sudakov_th_gqqH(N,xp));
      else ris.push_back((0.,0.));
      if ((_channel==0)||(_channel==3)) 
	ris.push_back(LOqqgH(N,xp)*(1.+_as*Hth1qqgH(xp))*Sudakov_th_qqgH(N,xp));
      else ris.push_back((0.,0.));
      break;
    }
  }
...
std::vector<std::complex<long double>> FixedptResum::ComputeMatching(std::complex<long double> N, long double xp){
  ...
  switch (_ordres){
	...
	case(2):{
	  if ((_channel==0)||(_channel==1)) 
	    match.push_back(LOgggH(N,xp)*(1.+3./8.*Ath1g*lN*lN*_as));
	  else match.push_back((0.,0.));
	  if ((_channel==0)||(_channel==2)) 
	    match.push_back(LOgqqH(N,xp)*(1.+1./8.*(2.*Ath1g+Ath1q)*lN*lN*_as));
	  else match.push_back((0.,0.));
	  if ((_channel==0)||(_channel==3)) 
	    match.push_back(LOqqgH(N,xp)*(1.+1./8.*(4.*Ath1q-Ath1g)*lN*lN*_as));
	  else match.push_back((0.,0.));
	  break;
	}
	case(3):{
	  if ((_channel==0)||(_channel==1)) 
	    match.push_back(LOgggH(N,xp)*(1.+3./8.*Ath1g*lN*lN*_as+1./384.*Ath1g*lN*lN*lN*(27.*Ath1g*lN+40.*beta_0)*_as*_as));
	  else match.push_back((0.,0.));
	  if ((_channel==0)||(_channel==2)) 
	    match.push_back(LOgqqH(N,xp)*(1.+1./8.*(2.*Ath1g+Ath1q)*lN*lN*_as
	    +1./384.*lN*lN*lN*(3.*std::pow(2.*Ath1g+Ath1q,2)*lN+8.*(4.*Ath1g+Ath1q)*beta_0)*_as*_as));
	  else match.push_back((0.,0.));
	  if ((_channel==0)||(_channel==3)) 
	    match.push_back(LOqqgH(N,xp)*(1.+1./8.*(4.*Ath1q-Ath1g)*lN*lN*_as
	    +1./384.*lN*lN*lN*(3.*std::pow(Ath1g-4.*Ath1q,2)*lN+8.*(-3.*Ath1g+8.*Ath1q)*beta_0)*_as*_as));
	  else match.push_back((0.,0.));
	  break;
	}
      }
	...
	case(2):{
	  if ((_channel==0)||(_channel==1)) 
	    match.push_back(LOgggH(N,xp)*(1.+_as*(Hth1gggH(xp)+1./8.*lN*(-4.*Bth1g-8.*Ath1g*_LF
	    +3.*Ath1g*lN+4.*Ath1g*std::log(radxp)))));
	  else match.push_back((0.,0.));
	  if ((_channel==0)||(_channel==2)) 
	    match.push_back(LOgqqH(N,xp)*(1.+_as*(Hth1gqqH(xp)+1./8.*lN*(-4.*(Bth1q+(Ath1g+Ath1q)*_LF)
	      +(2.*Ath1g+Ath1q)*lN+4.*Ath1q*std::log(radxp)))));
	  else match.push_back((0.,0.));
	  if ((_channel==0)||(_channel==3)) 
	    match.push_back(LOqqgH(N,xp)*(1.+_as*(Hth1qqgH(xp)+1./8.*lN*(-4.*Bth1g-8.*Ath1q*_LF
	...
	case(3):{
	  if ((_channel==0)||(_channel==1)) 
	    match.push_back(LOgggH(N,xp)*(1.+_as*(Hth1gggH(xp)+1./8.*lN*(-4.*Bth1g-8.*Ath1g*_LF
	    +3.*Ath1g*lN+4.*Ath1g*std::log(radxp)))+_as*_as*(1./8.*(Hth1gggH(xp)*lN*
	    (-4.*Bth1g-8.*Ath1g*_LF+3.*Ath1g*lN+4.*Ath1g*std::log(radxp))+1./48.*lN*lN*(3.*
	      std::pow(4.*Bth1g+8.*Ath1g*_LF-3.*Ath1g*lN-4.*Ath1g*std::log(radxp),2.)
	      +8.*(18.*Ath2g+(-6.*Bth1g+5.*Ath1g*lN-18.*Ath1g*_LR)*beta_0+12.*Ath1g*beta_0*std::log(radxp)))))));
	  else match.push_back((0.,0.));
	  if ((_channel==0)||(_channel==2)) 
	    match.push_back(LOgqqH(N,xp)*(1.+_as*(Hth1gqqH(xp)+1./8.*lN*(-4.*(Bth1q+(Ath1g+Ath1q)*_LF)
	      +(2.*Ath1g+Ath1q)*lN+4.*Ath1q*std::log(radxp)))+_as*_as*
	      (1./8.*(Hth1gqqH(xp)*lN*(-4.*(Bth1q+(Ath1g+Ath1q)*_LF)+(2.*Ath1g+Ath1q)*lN+4.*Ath1q*std::log(radxp))
		+1./48.*lN*lN*(3.*std::pow(-4.*(Bth1q+(Ath1g+Ath1q)*_LF)+(2.*Ath1g+Ath1q)*lN+4.*Ath1q*std::log(radxp),2.)
		+8.*(12.*Ath2g+6.*Ath2q+(-6.*Bth1q+4.*Ath1g*lN+Ath1q*lN-6.*(2.*Ath1g+Ath1q)*_LR)*beta_0
		+12.*Ath1q*beta_0*std::log(radxp)))))));
	  else match.push_back((0.,0.));
	  if ((_channel==0)||(_channel==3)) 
	    match.push_back(LOqqgH(N,xp)*(1.+_as*(Hth1qqgH(xp)+1./8.*lN*(-4.*Bth1g-8.*Ath1q*_LF
	      -Ath1g*lN+4.*Ath1q*lN+4.*Ath1g*std::log(radxp)))+_as*_as*
	      (1./8.*(Hth1qqgH(xp)*lN*(-4.*Bth1g-8.*Ath1q*_LF-Ath1g*lN+4.*Ath1q*lN+4.*Ath1g*std::log(radxp))
		+1./48.*lN*lN*(3.*std::pow(4.*Bth1g+8.*Ath1q*_LF+Ath1g*lN-4.*Ath1q*lN-4.*Ath1g*std::log(radxp),2.)
		+8.*(-6.*Ath2g+24.*Ath2q+(-6.*Bth1g-3.*Ath1g*lN+8.*Ath1q*lN+6.*(Ath1g-4.*Ath1q)*_LR)*beta_0
